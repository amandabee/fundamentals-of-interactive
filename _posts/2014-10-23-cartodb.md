...
-----
As I was walking through our merge query this morning, I realized we were going about it backwards. There's an easier way to approach the query. If you think of yourself as having two tables, one data table and one geography table, you can think of the query we ran to day as adding a column to our geography table and filling that column with data from the data table. 

But ... when you create any table on CartoDB, their system always adds a geography column, called `the_geom` and fills it with *null* if your table doesn't have obvious geographical information. So we could re-organize our query so that we're filling the geography column in our data table with geographic information. In our BLS example, if your state boundary file is called `us_states` and your BLS data file is called `bls_fatality_2011` and both tables include a column called `state` that you can use to join the two, this will work just fine:
